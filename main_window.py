...
class phone_log:
    def __init__(self, username):
        ...
        try:
            self.labels_pic = Image.open("required/delete.png")
        ...
        global label
        label = Button(self.menu, cursor='hand2', text=' Bin ', compound='left', relief=FLAT, bg='#E0F8C8', fg='black', activebackground='light grey', image=self.labels, font=('Cambria', 13, 'bold'), command=self.labels_page)
    ...
    def settings_page(self):
        ...
        try:
            self.f = open('required/sort.txt', 'r')
            self.content = self.f.read()
            self.uname_index = self.content.index(self.user)
            self.f.seek(self.uname_index)
            self.chosen = self.f.readline()
            self.f.close()
            self.choice = self.chosen.strip()
        except:
            self.choice = 'Name'
        ...
        self.sortby['values'] = ('Name', 'Random', 'No Sort')
        
        if self.choice.endswith('Name'):
            self.sortby.current(0)

        elif self.choice.endswith('Random'):
            self.sortby.current(1)
        
        elif self.choice.endswith('No Sort'):
            self.sortby.current(2)
        ...
        howtouse = Button(settings_window, text='❓ How to use', cursor='hand2', width=30, relief=FLAT, bg='#ECCCD2', font=('Arial', 17, 'bold'), command=lambda: os.startfile('how_to_use.mp4'))
        ...
        self.save_changes = Button(settings_window, cursor='hand2', text='Save Changes', bg='#F2F727', fg='#5034E3', width=15, font=('Arial', 15, 'bold'), command=self.save_changes_to_db)
        ...
        self.switch_account = Button(settings_window, width=20, height=1, relief=GROOVE, text='Switch Account', cursor='hand2', bg='#C7FFF5', fg='#B90000', font=('Cambria', 16, 'bold'), command=self.open_signin)
        self.switch_account.place(x=520, y=60)

        self.new_account = Button(settings_window, width=20, height=1, relief=GROOVE, text='Create New Account', cursor='hand2', bg='#C7FFF5', fg='#B90000', font=('Cambria', 16, 'bold'), command=self.open_signup)
        self.new_account.place(x=520, y=120)

        self.view_mytab = Button(settings_window, width=20, height=1, relief=GROOVE, text='Show my details', cursor='hand2', bg='#C7FFF5', fg='#B90000', font=('Cambria', 16, 'bold'), command=self.your_profile)
        self.view_mytab.place(x=520, y=180)

        self.labelstab = Button(settings_window, width=20, relief=GROOVE, height=1, text='Show deleted Contacts', cursor='hand2', bg='#C7FFF5', fg='#B90000', font=('Cambria', 16, 'bold'), command=self.labels_page)
        self.labelstab.place(x=520, y=240)
        ...
        self.clear_contacts = Button(settings_window, width=16, text='Clear Contacts', cursor='hand2', bg='#EE3556', fg='#0F0572', font=('Century', 14, 'bold'), command=self.clearContacts)
    ...
    def clearContacts(self):
        self.yesno = messagebox.askquestion('Continue?', 'Are you sure you want to delete all contacts?')
        if self.yesno == 'yes':
            try:
                self.connect = sq.connect(database='phone_log.db')
                self.c_cursor = self.connect.cursor()
                self.c_cursor.execute(f"""DROP TABLE {self.user}""")
                self.c_cursor.execute(f"""CREATE TABLE {self.user}(CONTACT, pic, phone1, phone2, phone3, email, address, birthday, notes)""")
                self.connect.commit()
                self.connect.close()
            except:
                pass
    ...
    def contacts_page(self):
        ...
        global o
        o = self.Scrollable_frame(self.root, self.user)
        o.pack(side=LEFT)
    ...
    def labels_page(self):
        ...
        labels_window = self.user_bin(self.root, self.user)
        labels_window.pack(side=LEFT)
    ...
    def opendeleteaccount(self):
        self.root.destroy()
        delete_account.delete_window(username=self.user)
    ...
    def save_changes_to_db(self):
        ...
        self.file.write(self.user + ' : ' + self.choice + '\n')
        self.file.close()

        messagebox.showinfo('Changes saved', 'Successfully saved changes..')
        contact.invoke()
    ...
    class Scrollable_frame(tk.Frame):
        ...
        def user_contacts(self):
            ...
            self.search_pic = Image.open('required/search.png')
            self.search_pic_resize = self.search_pic.resize((40, 40), Image.ANTIALIAS)
            self.search_resized_pic = ImageTk.PhotoImage(self.search_pic_resize)
            ...
            global search_cnt
            search_cnt = Button(self.frame, image=self.search_resized_pic, compound=LEFT, font=('Cambria', 15, 'bold'), bg='white', relief=FLAT, fg='dark blue', borderwidth=3, anchor='w', command=self.search_for_contact)
            search_cnt.place(x=50, y=0)
            ...
            try:
                ...
                if self.choice.endswith('Name'):
                    self.get_contacts = self.cursor.execute(f'SELECT CONTACT, EMAIL, PIC FROM {self.user} ORDER BY CONTACT')
                    self.cts = self.cursor.fetchall()
                
                elif self.choice.endswith('Random'):
                    self.get_contacts = self.cursor.execute(f'SELECT CONTACT, EMAIL, PIC FROM {self.user} ORDER BY RANDOM()')
                    self.cts = self.cursor.fetchall()
            ...
            if len(self.contact_names) == 0:
                Label(self.frame, text='Your contacts will appear here', bg='white', fg='grey', font=('Calibri', 13, 'italic')).pack(pady=10)
        ...
        def delete_contact(self, contactname):
            ...
            if self.choice == 'yes':
                try:
                    ...
                    self.all_details = self.cr.execute(f"""SELECT * FROM {self.user} WHERE CONTACT='{contactname}'""")
                    self.all_details = self.cr.fetchall()
                    self.details = self.all_details[0]

                    self.comm = f"""INSERT INTO {self.user}_bin (CONTACT, PIC, PHONE1, PHONE2, PHONE3, EMAIL, ADDRESS, NOTES) VALUES(?, ?, ?, ?, ?, ?, ?, ?)"""
                    self.vals = (self.details[0], self.details[1], self.details[2], self.details[3], self.details[4], self.details[5], self.details[6], self.details[8])
                    self.cr.execute(self.comm, self.vals)
        ...
        def search_for_contact(self):
            self.parent.destroy()
            search_contact.search_for_contact(self.user)

    class user_bin(tk.Frame):
        def __init__(self, parent, username):
            self.parent = parent
            self.user = username
            tk.Frame.__init__(self, parent)
        ...
        def user_contacts(self):
            Label(self.frame, text='Deleted Contacts', bg='white', fg='red', font=('Arial', 20, 'bold')).pack(pady=20)

            try:
                self.conn = sq.connect(database='phone_log.db')
                self.cursor = self.conn.cursor()
                self.get_contacts = self.cursor.execute(f'SELECT CONTACT, EMAIL, PIC FROM {self.user}_bin')
                self.cts = self.cursor.fetchall()
            ...
            self.label_img = Image.open('required/restore.png')
            self.resize_label_img = self.label_img.resize((25, 25), Image.ANTIALIAS)
            self.label_icon = ImageTk.PhotoImage(self.resize_label_img)


            if len(self.contact_names) == 0:
                Label(self.frame, text='Deleted contacts appear here', bg='white', fg='grey', font=('Calibri', 13, 'italic')).pack(pady=10)
            ...
            for widgets in range(len(self.contact_names)):
                ...
                self.restore = Button(self.contact_detail, relief=FLAT, image=self.label_icon, bg='#ECF3F3', fg='black')
                self.restore.pack(side=RIGHT, padx=10)
                ...
                self.c_pic['command'] = lambda uname=self.contact_names[widgets]: self.show_details(uname)
                self.delete['command'] = lambda uname=self.contact_names[widgets]: self.delete_contact(uname)
                self.restore['command'] = lambda uname=self.contact_names[widgets]: self.restore_contact(uname)
        ...
        def show_details(self, name):
            view_contact.show_contact(self.user, name, d='gagan')

        def delete_contact(self, contactname):
            self.choice = messagebox.askquestion('Warning', 'Are you sure you want to delete this contact from the bin?')
            if self.choice == 'yes':
                try:
                    self.c = sq.connect(database='phone_log.db')
                    self.cr = self.c.cursor()
                    self.delete_ctc = self.cr.execute(f"""DELETE FROM {self.user}_bin WHERE CONTACT='{contactname}'""")
                    self.c.commit()
                    self.c.close()
        ...
        def restore_contact(self, contactname):
            try:
                    self.c = sq.connect(database='phone_log.db')
                    self.cr = self.c.cursor()
                    self.all_details = self.cr.execute(f"""SELECT * FROM {self.user}_bin WHERE CONTACT='{contactname}'""")
                    self.all_details = self.cr.fetchall()
                    self.details = self.all_details[0]

                    self.comm = f"""INSERT INTO {self.user} (CONTACT, PIC, PHONE1, PHONE2, PHONE3, EMAIL, ADDRESS, NOTES) VALUES(?, ?, ?, ?, ?, ?, ?, ?)"""
                    self.vals = (self.details[0], self.details[1], self.details[2], self.details[3], self.details[4], self.details[5], self.details[6], self.details[8])
                    self.cr.execute(self.comm, self.vals)
                    ...
                    messagebox.showinfo('Contact restored', 'Contact successfully restored')
                    label.invoke()
            except:
                pass
